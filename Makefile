...
$(GEN_TS_PATTERN) $(GEN_RS_PATTERN) $(GEN_SERDE_RS_PATTERN) $(GEN_TONIC_RS_PATTERN): $(PROTOS)
	@if [ "${BUF_VERSION}" != "1.50.0" ]; then echo "Warning: buf 1.50.0 is not installed! Please install v1.32.0 of the 'buf' command line tool: https://docs.buf.build/installation"; exit 1; fi;
	buf lint api
	buf generate -v api
